...
public class BatchProcessingCore {
	...
	public Boolean generateReport(ConsolidatedData consolidatedData, String fileName, File reportLocation)
			throws Exception {

		List<SellerData> sellerData = consolidatedData.getSellerData().stream()
				.filter(seller -> !seller.getCpf().isEmpty())
				.filter(distinctUtils.distinctByKey(seller -> seller.getCpf())).collect(Collectors.toList());

		List<ClientData> clientData = consolidatedData.getClientData().stream()
				.filter(client -> !client.getCnpj().isEmpty())
				.filter(distinctUtils.distinctByKey(client -> client.getCnpj())).collect(Collectors.toList());
		...
		sellerData.forEach(seller -> {
			...
			if (seller.getName() != null) {
				salesByName = salesData.stream().filter(sD -> !sD.getNameSalesman().isEmpty())
						.filter(sD -> sD.getNameSalesman().equals(seller.getName())).collect(Collectors.toList());
			}
